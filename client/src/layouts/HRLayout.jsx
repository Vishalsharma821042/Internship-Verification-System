...
import { LayoutDashboard, UserPlus, ShieldCheck, LogOut, Menu, X, ChevronRight, Search } from 'lucide-react';

function cn(...c) { return c.filter(Boolean).join(' '); }

const MENU = [
  { icon: LayoutDashboard, label: 'Dashboard',         path: '/hr/dashboard' },
  { icon: UserPlus,        label: 'Generate Code',     path: '/hr/generate' },
  { icon: Search,          label: 'Verify Internship', path: '/hr/verify' },
];
    ...
    <>
      ...
      <div className="px-5 py-5 border-b border-white/10">
        <Link to="/hr/dashboard" className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-white leading-tight">HR Portal</p>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-gray-500 px-3 py-2">Quick Actions</p>
      ...
      <div className="flex-1 md:ml-64 flex flex-col min-h-screen">
        <header className="h-14 sticky top-0 z-30 flex items-center justify-between px-6 md:px-8 border-b border-white/10 bg-black/85 backdrop-blur">
          <div className="flex items-center gap-2 md:ml-0 ml-10">
            ...
            <span className="text-xs text-gray-500 font-semibold uppercase tracking-widest">HR Console</span>
          </div>
